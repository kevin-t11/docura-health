...
/** Provide file selection, drag-and-drop, and attachment previews. */
...
type AttachmentRejectReason = 'too-large' | 'max-files';
type AttachmentUploadStatus = 'idle' | 'uploading' | 'complete' | 'failed';
...
type AttachmentUploadClassNames = {
  dropzone?: string;
  list?: string;
  row?: string;
};
...
function AttachmentIcon({ item }: { item: AttachmentUploadItem }) {
  const { kind } = item;
  ...
  return <DocumentIcon name={item.name} mimeType={item.file?.type} className="size-4" />;
}
    ...
    <Tooltip
      ...
      content={
        <span className="block w-32">
          ...
          {/* Local object URLs cannot use Next.js image optimization. */}
...
/** Stop preview interaction while its closing animation finishes. */
function ImagePreviewPresence({
  children
}: {
  children: (state: {
    isPresent: boolean;
...
function ImagePreviewDialog({
  ...
  const content =
    item && src ? (
      ...
      // edge. `ImagePreviewPresence` releases interaction in the same commit that starts
      // the exit.
      <ImagePreviewPresence>
        {({ isPresent, gate }) => (
          <div
            inert={!isPresent}
...
export function AttachmentUpload({
  ...
  const [previewSelection, setPreviewItem] = useState<AttachmentUploadItem | null>(null);
  ...
  useEffect(() => {
    ...
    const activeUrls = new Set(items.flatMap((item) => [item.href, item.previewUrl]));
  ...
  const previewItem = items.find((item) => item.id === previewSelection?.id) ?? null;
